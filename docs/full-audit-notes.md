...
## Backend
- CI commands (from workflow): `npm run typecheck`, `npm run lint`, `npm test -- --runInBand`, `npm run build` (with Postgres 16 service, `TEST_DATABASE_URL`, `ALLOW_TEST_DB_RESET`).
...
- Rerun backend and mobile `npm run typecheck`, `npm run lint`, `npm test -- --runInBand`, and `npm run build` once Node/npm are available.
