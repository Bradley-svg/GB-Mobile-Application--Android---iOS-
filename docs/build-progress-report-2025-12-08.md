...
- New: signed file URLs (HMAC via `FILE_SIGNING_SECRET`) layer on top of the existing JWT `/files` flow (`/files/:id/signed-url` -> `/files/signed/:token`) with mobile flag-off by default; AV scan tests now cover infected/error/no-persist cases plus a stubbed scanner command harness.
...
- Work orders/tasks/attachments/SLA: Status transitions enforced, SLA breach recomputed, checklist replace, maintenance summary buckets, attachments stored under `FILE_STORAGE_ROOT`. Antivirus scanning (stubbed in tests/when disabled) runs before persisting and rejects infected uploads; API tests now assert infected/error cases do not persist rows and clean temp files. Signed URLs can be minted via `/files/:id/signed-url` when `FILE_SIGNING_SECRET` is set, alongside the existing JWT `/files/:path` flow. Tests: `workOrders*.test.ts`, `workOrdersMaintenance.api.test.ts`, `workOrderAttachments.api.test.ts`, `workOrdersService.sla.test.ts`, `files.api.test.ts`, `fileUrlSigner.test.ts`.
- Documents/storage: Site/device upload + list, path sanitisation, public URL mapping. Antivirus scanning sits in front of uploads with the same stub-in-tests behaviour; signed URLs reuse `/files/:id/signed-url` + `/files/signed/:token`. Tests: `documents.api.test.ts`, `files.api.test.ts`.
...
- Issues/Risks
  - P1: AV scanning and signed URLs depend on env (`AV_SCANNER_*`, `FILE_SIGNING_SECRET`); prod/staging need real scanner targets plus secrets/TTL/CDN config.
...
- Work orders/Maintenance: List with SLA/due pills and filters, offline cached; detail (per tests) handles status transitions, checklist, attachments; maintenance calendar from `/maintenance/summary`. Optional signed URLs (`EXPO_PUBLIC_USE_SIGNED_FILE_URLS`) are fetched before opening attachments; default stays JWT `/files`. Risks: offline actions disabled but caches rely on previous session.
- Documents/vault: Site/device documents list with category/status pills, opens via API base URL; optional signed URLs when the feature flag is enabled; offline read-only banner when cached; no mobile upload UI.
...
- **P1:**
  1) Configure AV_SCANNER_* + FILE_SIGNING_SECRET in prod/staging and decide CDN/TTL defaults for signed URLs.
- **P2:**
