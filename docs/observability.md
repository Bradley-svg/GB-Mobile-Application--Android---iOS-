...
- `/health-plus.heatPumpHistory`: page when `configured:true` and either `lastSuccessAt` is stale (e.g., >30m) or `lastErrorAt` advances without a newer success; downgrade to warn when `disabled:true` or `vendorFlags.disabled` includes `HEATPUMP_HISTORY_DISABLED` (CI/offline/demo).
- Logs: alert on spikes for `module:heatPumpHistory` with messages `heat pump history circuit open` or `heat pump history upstream error` (includes the `circuit opened` variant); apply the same vendor-flag guard before paging and capture `status`/`bodyPreview` fields.
