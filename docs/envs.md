...
- Demo tenant: `DEMO_EMAIL`/`DEMO_PASSWORD` (seeded owner login), `DEMO_ORG_ID` (defaults to `11111111-1111-1111-1111-111111111111`), `DEMO_DEVICE_MAC` (defaults to `38:18:2B:60:A9:94` for history alignment).
...
- CORS: `CORS_ALLOWED_ORIGINS` (legacy) / `WEB_ALLOWED_ORIGINS` (preferred) control browser origins; keep explicit lists outside dev. Examples: `http://localhost:3000,http://localhost:19006` for dev, `https://app.greenbro.co.za,https://www.greenbro.co.za` for production.

## Staging
- Base: `NODE_ENV=production`, `APP_VERSION=0.8.0`, `PORT=4000`, `WEB_ALLOWED_ORIGINS` / `CORS_ALLOWED_ORIGINS` listing staging web/app origins (include the staging app host and the WordPress staging domain if framed) plus matching `FRAME_ANCESTORS` when embeds are enabled.
...
- Demo tenant: set `DEMO_EMAIL=demo@greenbro.com`, `DEMO_PASSWORD=<staging-demo-pass>`, `DEMO_USER_PASSWORD=<staging-demo-pass>`, `DEMO_ORG_ID=11111111-1111-1111-1111-111111111111`, `DEMO_DEVICE_MAC=38:18:2B:60:A9:94` to match the seeded org/user/device.
...
- Integrations: configure MQTT/control/history (`MQTT_URL`, `MQTT_USERNAME`, `MQTT_PASSWORD`, `MQTT_TELEMETRY_TOPIC`, `MQTT_CONTROL_TOPIC_TEMPLATE`, `CONTROL_API_URL`, `CONTROL_API_KEY`, `HEATPUMP_HISTORY_URL`, `HEATPUMP_HISTORY_API_KEY`, `HEATPUMP_HISTORY_TIMEOUT_MS`) or explicitly set the disable flags when offline. Staging defaults stay safe: history stays enabled (`HEATPUMP_HISTORY_DISABLED=false`) while commands/push paths are off by default (`MQTT_DISABLED=true`, `CONTROL_API_DISABLED=true`, `PUSH_NOTIFICATIONS_DISABLED=true`) unless you are intentionally testing them.
- Push: `EXPO_ACCESS_TOKEN=<staging-expo-token>`, `PUSH_NOTIFICATIONS_ENABLED_ROLES=owner,admin,facilities`, `PUSH_NOTIFICATIONS_DISABLED=true` unless actively testing notifications; optional `PUSH_HEALTHCHECK_ENABLED`/`PUSH_HEALTHCHECK_TOKEN` for sample pushes.
...
## Production
- Base: `NODE_ENV=production`, `APP_VERSION=0.8.0`, `PORT=4000`, CORS allowlist restricted to trusted production domains (`WEB_ALLOWED_ORIGINS=https://app.greenbro.co.za,https://www.greenbro.co.za`).
...
- Demo tenant: set `DEMO_EMAIL=demo@greenbro.com`, `DEMO_PASSWORD=<prod-demo-pass>`, `DEMO_ORG_ID=11111111-1111-1111-1111-111111111111`, `DEMO_DEVICE_MAC=38:18:2B:60:A9:94` if a shared demo tenant is required.
...
- `NEXT_PUBLIC_API_URL` (web): dev `http://localhost:4000`, staging `https://staging.api.greenbro.co.za`, production `https://api.greenbro.co.za`.
- `NEXT_PUBLIC_EMBEDDED` (web): optional boolean to allow iframe embedding with `?embed=true` for the marketing site.
- Web session/security: `NEXT_PUBLIC_AUTH_STORAGE_MODE=local-storage|cookie` (cookie = future httpOnly mode), `NEXT_PUBLIC_AUTH_2FA_ENABLED` (mirror backend toggle), `NEXT_PUBLIC_SESSION_IDLE_MINUTES` (default 30), `NEXT_PUBLIC_SESSION_ABSOLUTE_HOURS` (default 8).
- Web E2E: `WEB_E2E_EMAIL`/`WEB_E2E_PASSWORD` defaults to the seeded demo user; `WEB_E2E_IDLE_TEST`/`WEB_E2E_IDLE_WAIT_MS` opt into the idle-timeout Playwright check.
