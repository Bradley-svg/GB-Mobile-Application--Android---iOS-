...
- Work orders (0.4.x): new Work Orders list/detail screens (App stack) with offline cached list, linked-alert creation entry in Alert Detail, and Profile shortcut. Detail supports status actions, basic notes field, and checklist toggles that call `/work-orders/:id` + `/work-orders/:id/tasks`; Alert Detail can create and jump to a linked work order when online.
