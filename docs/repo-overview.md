...
- Alerts/control scheduling (0.3.x): `alert_rules` table (threshold/ROC/offline) now drives the alerts worker with periodic rule refresh; load-shedding/TOU windows live in `site_schedules`; device setpoint schedules in `device_schedules` exposed via `/devices/:id/schedule`; control audit available via `/devices/:id/commands`; alerts worker publishes rule engine stats into `/health-plus`.
