...
## Branding
- Branding source of truth: `docs/branding/README.md` (logo rules + palette). Master artwork lives at `docs/branding/official/greenbro-logo-horizontal-gearO.svg` with exports in `docs/branding/official/` and `mobile/assets/greenbro/` (gear-as-O logo, splash, icon).
...
- Alerts/control scheduling (0.3.x): `alert_rules` table (threshold/ROC/offline) now drives the alerts worker with periodic rule refresh; load-shedding/TOU windows live in `site_schedules`; device setpoint schedules in `device_schedules` exposed via `/devices/:id/schedule`; control audit available via `/devices/:id/commands`; alerts worker publishes rule engine stats into `/health-plus`. New rule/schedule/command routes are auth-protected, org-scoped, and keep failed/throttled control attempts in the audit trail.
...
- Work orders (0.5.x): work_orders, work_order_tasks, and work_order_attachments tables plus org-scoped routes (/work-orders, /alerts/:id/work-orders, /devices/:id/work-orders, /sites/:id/work-orders) with simple status transitions (open > in_progress > done/cancelled) and task replacement. Attachments now upload via /work-orders/:id/attachments to FILE_STORAGE_ROOT (dev static served from /files) with filename/mime/size metadata. Creating from an alert seeds a default checklist tied to the alert/device/site; SLA metadata (sla_due_at, resolved_at, sla_breached, reminder_at, category) is additive and recalculated on reads/writes, and /maintenance/summary surfaces org-scoped open/overdue/dueSoon counts bucketed by SLA date. A site/device document vault is exposed via /sites/:id/documents and /devices/:id/documents.
- Health (2025-12-07 local): typecheck/lint/test/build all green with TEST_DATABASE_URL=postgres://postgres:postgres@localhost:5432/greenbro_test and ALLOW_TEST_DB_RESET=true against Postgres 16. Last recorded /health-plus sample (2025-12-05) showed mqtt/configured:false healthy:true; control configured:false healthy:true with CONTROL_CHANNEL_UNCONFIGURED; heatPumpHistory configured:true healthy:false while upstream was idle; alertsWorker healthy:true; push enabled:false with lastSampleAt present. Maintenance snapshot now returns open/overdue counts alongside the existing db/mqtt/control/alertsWorker/push payload, plus a storage root/writable check.
...
- Fleet visibility/search (0.2.x): `healthScoreService` normalises connectivity/last_seen/alert severity into `healthy|warning|critical|offline`. `/sites` + `/sites/:id` return `health` and a `last_seen` summary; `/sites/:id/devices` exposes the same for devices; `/fleet` supports organisation-scoped search with `q`, `health`, and future `tag` filters (limit/offset ready).
...
- Alerts/control polish (0.3.x): connectivity pills on Dashboard/Site/Device with firmware text, Alert Detail snooze chips pick up rule defaults (incl. until resolved cap), Device Detail schedule card and control history wired to `/devices/:id/schedule` + `/devices/:id/commands` with offline read-only state, Diagnostics shows alertsEngine metrics.
- Work orders (0.5.x): work_orders, work_order_tasks, and work_order_attachments tables plus org-scoped routes (/work-orders, /alerts/:id/work-orders, /devices/:id/work-orders, /sites/:id/work-orders) with simple status transitions (open > in_progress > done/cancelled) and task replacement. Attachments now upload via /work-orders/:id/attachments to FILE_STORAGE_ROOT (dev static served from /files) with filename/mime/size metadata. Creating from an alert seeds a default checklist tied to the alert/device/site; SLA metadata (sla_due_at, resolved_at, sla_breached, reminder_at, category) is additive and recalculated on reads/writes, and /maintenance/summary surfaces org-scoped open/overdue/dueSoon counts bucketed by SLA date. A site/device document vault is exposed via /sites/:id/documents and /devices/:id/documents.
